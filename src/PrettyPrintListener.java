...
public class PrettyPrintListener extends exprBaseListener {
    ...
    public void exitRelationalExpression(exprParser.RelationalExpressionContext ctx) {
        if (ctx.LT() != null || ctx.GT() != null || ctx.LE() != null || ctx.GE() != null) {
            if (stack.size() < 2) {
                throw new RuntimeException("Not enough operands for relational expression");
            }
            int right = (int) stack.pop();
            int left = (int) stack.pop();
            if (ctx.LT() != null) stack.push(left < right ? 1 : 0);
            else if (ctx.GT() != null) stack.push(left > right ? 1 : 0);
            else if (ctx.LE() != null) stack.push(left <= right ? 1 : 0);
            else if (ctx.GE() != null) stack.push(left >= right ? 1 : 0);
        }
    }
    @Override
